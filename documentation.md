...
5. `POST /api/auth/refresh` (public route) reads the refresh cookie, validates and rotates the DB session inside a transaction with row locking, and returns a new access token JSON payload plus a new cookie.
