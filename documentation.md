...
| `app/Services/`                        | `PatrolSessionSummaryService` (Milestone 12; read-only summary), `PatrolValidationService` (Milestone 1; gap-aware validation engine, persists checkpoint results)                                                                                                                                                                                                                                                                                                         |
...
**Anti-cheat (per segment only):** speed > **41.67 m/s** (~150 km/h) without gap; GPS jump **> 100 m** within **≤ 5 s** without gap; poor accuracy **> 50 m**; timestamp missing/duplicate/out-of-order. Speed/jump rules **do not** apply across gaps.
...
- Client `**manual`** `**source**` is stored as `**sync**` (enum constraint).
